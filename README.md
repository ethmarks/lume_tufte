...
- [x] Clone this repo or click the "Use this template" button on GitHub.
- [x] Update dependencies with `deno task lume upgrade`.
- [x] Add or modify the files of the theme in the `src` folder.
- [x] Edit the `plugins.ts` file to add more plugins to your theme.
- [ ] Edit the `_cms.ts` file to configure Lume CMS.
- [ ] Enter in the `test` folder and run `deno task serve` to test the theme.
- [x] Edit the `LICENSE` file to set the author name and year.
...
- [ ] Publish the theme on GitHub, add a tag so it can be consumed from
      JsDelivr.
